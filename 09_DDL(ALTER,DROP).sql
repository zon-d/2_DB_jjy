...
ALTER TABLE DEPT_COPY
MODIFY DEPT_TITLE NOT NULL; -- DEPT_TITLE 컬럼을 NOT NULL로 제어
...
-- ALTER TABLE 테이블명 MODIFY 컬럼명 NULL/NO NULL; -> NULL 여부 변경

-- 컬럼 삭제
-- ALTER TABLE 테이블명 DROP(삭제할 컬럼명);
-- ALTER TABLE 테이블명 DROP DROP COLUMN 삭제할 컬럼명;
...
-- ORA-12899: "KH_JJY"."DEPT_COPY"."DEPT_ID" 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)

-- DEPT_ID 컬럼 (CHAR(2)라서 'D10'이 입력이 안되기때문에)
ALTER TABLE DEPT_COPY MODIFY DEPT_ID VARCHAR(3);
--> 컬럼 변경 후 다시 위 INSERT 실행 -> 성공
...
-- ORA-12983: 테이블에 모든 열들을 삭제할 수 없습니다 * 테이블 당 컬럼 1개씩은 있어야해서
...
CREATE TABLE DEPT_COPY
AS SELECT * FROM DEPARTMENT;
--> 컬럼명, 데이터타입, NOT NULL 여부만 복사됨 *제약조건이 전부 복사되지 않음
...
INSERT INTO TB2 VALUES(14,4); -- 삽입안됨. TB1에 4가 없어서

-- TB1 삭제
DROP TABLE TB1;
-- ORA-02449: 외래 키에 의해 참조되는 고유/기본 키가 테이블에 있습니다 * TB2가 TB1을 참조하고있어서
--> 해결방법
-- 1) 자식, 부모 테이블 순서로 삭제
-- 2) ALTER를 이용해서 FK 제약조건 삭제후 TB1 삭제
-- 3) DROP TABLE 삭제옵선 CASCADE CONSTRAINTS 사용

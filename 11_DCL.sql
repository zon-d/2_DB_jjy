...
* 사용자 계정 : 데이터베이스에 대하여 질의, 갱신, 보고서 작성 등의
                ...
                ex) KH_jjy계정(각자 이니셜 계정), workbook 등
*/
...
CREATE USER jjy_sample IDENTIFIED BY sample1234;
...
-- [권한 부여 작성법]
-- GRANT 권한, 권한, .... TO 사용자명;
GRANT CREATE SESSION TO jjy_sample;
...
--> CREATE TABLE : 테이블 생성 권한
-- + 데이터를 저장할 수 있는 공간(TABLESPACE)

-- 6. (SYS) 테이블 생성 권한 + TABLESAPCE 할당
GRANT CREATE TABLE TO jjy_sample;

ALTER USER jjy_sample DEFAULT TABLESPACE
...
---> 해당 계정은 지정된 권한을 이용해서 특정 역할을 할 수 있다.

-- (SYS) sample 계정에 CONNECT, RESOURCE 부여
GRANT CONNECT, RESOURCE TO jjy_sample;
-- CONNECT : DB 접속 권한을 묶어둔 ROLE
...
-- kh_jjy   /   jjy_sample 사용자 계정끼리 서로 객체 접근 권한 부여

-- 1. (sample) kh_jjy 계정의 EMPLOYEE 테이블 조회
SELECT * FROM kh_jjy.EMPLOYEE;
-- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다
--> 접근 권한이 없어서 조회 불가

-- 2. (kh) jjy_sample 계정에 EMPLOYEE 테이블 조회 권한 부여

-- [객체 권한 부여 방법]
-- GRANT 객체권한 ON 객체명 TO 사용자명;
GRANT SELECT ON EMPLOYEE TO jjy_sample;

-- 3. (sample) 다시 kh_jjy.EMPLOYEE 조회
SELECT * FROM kh_jjy.EMPLOYEE;

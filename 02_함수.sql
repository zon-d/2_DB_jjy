...
SELECT EMP_NAME, SUBSTR(EMAIL, 1,  INSTR(EMAIL, '@') -1 ) 아이디 -- -1을 적어야 @가 사라짐
FROM EMPLOYEE;
...
SELECT TRUNC(123.455, -1) FROM DUAL; -- 120, 10의 자리 아래 버림
...
SELECT ROUND (MONTHS_BETWEEN(SYSDATE ,'1997-06-15')) "정지윤 개월수"
FROM DUAL;

SELECT ROUND (MONTHS_BETWEEN(SYSDATE, '2021-06-13')) ||'개월' "철수 개월수"
FROM DUAL;
...
SELECT TO_CHAR( SYSDATE, 'MM/DD (DY)') FROM DUAL; -- 09/01 (목)
...
SELECT EMP_NAME,
	...
	-- (위 코드와 동일한 결과) SELECT EMP_NAME, SUBSTR(EMP_NO, 1, 6)
FROM EMPLOYEE;
...
/* NULL과 산술 연살을 진행하면 결과는 무조건 NULL */
...
-- 직원의 성별 구하기 (남 : 1 / 여 : 2)
SELECT EMP_NAME AS 이름, DECODE ( SUBSTR(EMP_NO, 8, 1) , '1', '남성' , '2', '여성') AS 성별
FROM EMPLOYEE;
...
-- AVG(숫자가 기록된 컬럼명)  : 평균
-- 전 직원 급여 평균
SELECT ROUNDSELECT SUM(SALARY) FROM EMPLOYEE; -- 3047663
...
SELECT COUNT (DISTINCT DEPT_CODE) FROM EMPLOYEE; -- 6헹

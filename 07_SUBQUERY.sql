...
SLECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
...
/*  서브쿼리 유형
    ...
    - 단일행 (단일열) 서브쿼리 : 서브쿼리의 조회 결과 값의 개수가 1개일 때  조회결과:1행1열
...
SELECT EMP_ID, EMP_NAME, DPET_TITLE, JOB_NAME
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID);
...
-- 2000년 입사 추출
-- WHERE EXTRACT(YEAR FROM IRE_DATE) = 2000;
...
-- 2) 직급별 평균 급여
SELECT JOB_CODE, CEIL(AVG(SALARY))
FROM EMPLOYEE
GROUP BY JOB_CODE
...
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
WHERE (JOB_CODE, SALARY) IN (SELECT JOB_CODE, CEIL(AVG(SALARY))
							FROM EMPLOYEE
							GROUP BY JOB_CODE)
...
-- 1) MAIN 의 1행의 DEPT_CODE를 SUB에 대입
-- 2) SUB를 수행

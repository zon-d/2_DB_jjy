...
FROM EMPLOYEE
JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE); --연결고리 역할의 컬럼


-- 1. 내부 조인(INNER JOIN) ( == 등가 조인(EQUAL JOIN)*등호(=)들어가면 해당)
...
-- 3) FULL [OUTER] JOIN   : 합치기에 사용한 두 테이블이 가진 모든 행을 결과에 포함 (왼쪽+오른쪽)
...
-- 3. 교차 조인(CROSS JOIN == CARTESIAN PRODUCT) *경우의 수 확인
...
--> EMPLOYEE 테이블에 LOCATION_ID 컬럼이 없어서 오류
